...
* Fixed: The document download always failed with a blank page because it fetched connector settings from the legacy `get_option( $slug )` location instead of the resolved settings already available on the class, which are empty on sites that migrated to the nested `connect_ecommerce` option (e.g. after the Holded connector's settings migration).
